...
import com.nuggylib.enchantmentsseer.client.render.EnchantmentsSeerRenderer.Model3D.SpriteInfo;
 ...
 * @see "https://github.com/mekanism/Mekanism/blob/v10.1/src/main/java/mekanism/client/render/RenderResizableCuboid.java"
 */
public class RenderResizableCuboid {
    ...
    private static void putTexturedQuad(IVertexBuilder buffer, Matrix4f matrix, Matrix3f normalMatrix, @Nullable SpriteInfo spriteInfo, Vector3f from, Vector3f to,
        ...
        float red = EnchantmentsSeerRenderer.getRed(argb);
        float green = EnchantmentsSeerRenderer.getGreen(argb);
        float blue = EnchantmentsSeerRenderer.getBlue(argb);
        float alpha = EnchantmentsSeerRenderer.getAlpha(argb);
        // add quads
        switch (face) {
            case DOWN:
