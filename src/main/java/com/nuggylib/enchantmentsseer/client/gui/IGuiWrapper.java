...
public interface IGuiWrapper {
    ...
    default void addWindow(GuiWindow window) {
        EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'addWindow' but unsupported in {}", getClass().getName());
    }

    default void removeWindow(GuiWindow window) {
        EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'removeWindow' but unsupported in {}", getClass().getName());
    }
    ...
    default GuiWindow getWindowHovering(double mouseX, double mouseY) {
        EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'getWindowHovering' but unsupported in {}", getClass().getName());
        return null;
    }

    @Nonnull
    default BooleanSupplier trackWarning(@Nonnull WarningType type, @Nonnull BooleanSupplier warningSupplier) {
        EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'trackWarning' but unsupported in {}", getClass().getName());
    ...
    default void renderItemTooltip(MatrixStack matrix, @Nonnull ItemStack stack, int xAxis, int yAxis) {
        EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'renderItemTooltip' but unsupported in {}", getClass().getName());
    }
    ...
    default void renderItemTooltipWithExtra(MatrixStack matrix, @Nonnull ItemStack stack, int xAxis, int yAxis, List<ITextComponent> toAppend) {
        if (toAppend.isEmpty()) {
            ...
            EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'renderItemTooltipWithExtra' but unsupported in {}", getClass().getName());
        }
    }
    ...
    default void setSelectedWindow(SelectedWindowData selectedWindow) {
        EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'setSelectedWindow' but unsupported in {}", getClass().getName());
    }

    default void addFocusListener(GuiElement element) {
        EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'addFocusListener' but unsupported in {}", getClass().getName());
    }

    default void removeFocusListener(GuiElement element) {
        EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'removeFocusListener' but unsupported in {}", getClass().getName());
    }

    default void focusChange(GuiElement changed) {
        EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'focusChange' but unsupported in {}", getClass().getName());
    }

    default void incrementFocus(GuiElement current) {
        EnchantmentsSeer.MAIN_LOGGER.error("Tried to call 'incrementFocus' but unsupported in {}", getClass().getName());
    }
}
