...
import com.nuggylib.enchantmentsseer.api.Range3D;
 ...
 *
 * @see "https://github.com/mekanism/Mekanism/blob/v10.1/src/main/java/mekanism/common/network/BasePacketHandler.java"
 */
public abstract class BasePacketHandler {
    ...
    private static String getProtocolVersion() {
        return EnchantmentsSeer.instance == null ? "999.999.999" : EnchantmentsSeer.instance.versionNumber.toString();
    }
    ...
    protected <MSG extends IEnchantmentsSeerPacket> void registerClientToServer(Class<MSG> type, Function<PacketBuffer, MSG> decoder) {
        registerMessage(type, decoder, NetworkDirection.PLAY_TO_SERVER);
    }

    protected <MSG extends IEnchantmentsSeerPacket> void registerServerToClient(Class<MSG> type, Function<PacketBuffer, MSG> decoder) {
        registerMessage(type, decoder, NetworkDirection.PLAY_TO_CLIENT);
    }

    private <MSG extends IEnchantmentsSeerPacket> void registerMessage(Class<MSG> type, Function<PacketBuffer, MSG> decoder, NetworkDirection networkDirection) {
        getChannel().registerMessage(index++, type, IEnchantmentsSeerPacket::encode, decoder, IEnchantmentsSeerPacket::handle, Optional.of(networkDirection));
    }
